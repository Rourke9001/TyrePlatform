...
**Unticking is an observation, not an edit.** FR-INS-063 requires the system to permit a driver to begin with a composition that differs from the record. Unticking does that: the reduced set travels as `observed_member_vehicle_ids`, the server writes an `FR-INS-063` warning, and a controller reconciles it. **Adding** a trailer the record does not know about is deliberately *not* built — under FR-AUT-005 the driver has no scoped way to identify an arbitrary unit, and inventing a fleet-wide unit picker on a driver's phone would hand out exactly the configuration authority this design is keeping with the controller. That half of FR-INS-063 is partial. **Settled 25 Aug 2026 as decision D5** — the untick-yes / add-no split stands, and it is recorded in *What this plan does not build*, on epic TYRE-4, and in **SRS FR-INS-063 itself**, which now carries erratum D5 and says in the requirement text that it must not be closed on the untick path alone. (It is no longer in the questions file — that entry was deleted when the decision landed.) Do not close FR-INS-063 on the epic.
