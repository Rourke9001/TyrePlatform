...
UNITS = [
    ...
    (
        ...
        "HORSE",
        "CONFIRMED",
        [("STEER", False), ("DRIVE", True), ("DRIVE", True)],
    ),
    (
        "TRAILER_2AXLE",
        "2-axle trailer",
    ...
    (
        ...
        "TRAILER",
        "UNVERIFIED",
        [("TRAILER", True), ("TRAILER", True), ("TRAILER", True)],
    ),
    (
        "RIGID_4X2",
        "Rigid truck 4x2",
...
L = [
    "-- Seed: axle configuration library (unit types per CHG-070..073) + policy seeds",
...
L.append(
    "  ('11111111-1111-1111-1111-111111111111','BAC Transport','bac','ACTIVE','GENERATED'),"
)
L.append(
    "  ('22222222-2222-2222-2222-222222222222','Second Fleet (isolation control)','other','ACTIVE','FREE'),"
)
L.append(
    "  ('33333333-3333-3333-3333-333333333333','Sandbox Fleet','sandbox','ACTIVE','GENERATED');"
)
L.append("")
...
for tid in [
    ...
    L.append(
        "-- backdate, not load time: as-at valuation (FR-VAL-020/021) resolves the"
    )
    L.append("-- policy effective at the requested date, so policy must predate every")
    L.append(
        "-- seeded reading or historical dates would value as 'no policy configured'."
    )
    for k, v in [
        ...
        (
            "duplicate_inspection_min_hours",
            4,
        ),  # FR-INS-038's window, read by app.submit_inspection
        (
            "submitted_at_future_skew_minutes",
            5,
        ),  # TYRE-166: how far ahead of the server clock a device may stamp a submit
        (
            "wear_rate_alert_multiple",
            3,
        ),  # FR-INS-035, served to the device in the capture context
        ...
        L.append(
            f"INSERT INTO app.configuration (tenant_id,key,value,effective_from) VALUES ('{tid}','{k}','{json.dumps(v)}'::jsonb,'2024-01-01T00:00:00Z');"
        )
    L.append("")
    ...
    L.append("-- 4.0mm retread/scrap pull point cited from seed_policy.py (CFL-012).")
    L.append("-- A tenant's FIRST policy rows carry the sentinel -infinity (SRS §5.1,")
    L.append(
        "-- errata E1): the baseline asserts no date before which it did not apply,"
    )
    ...
    L.append(
        "-- change under it. Every deliberate change thereafter is a new dated row."
    )
    L.append(
        "INSERT INTO app.threshold_policy (id,tenant_id,retread_threshold_mm,scrap_threshold_mm,warning_threshold_mm,effective_from)"
    )
    L.append(
        f"  VALUES (md5('{tid}thrpol-default')::uuid,'{tid}',{RETREAD_THRESHOLD_MM},{SCRAP_THRESHOLD_MM},{WARNING_THRESHOLD_MM},'-infinity');"
    )
    L.append(
        "-- CHG-038: retreads are not fitted to steer axles. Fleet practice, not a"
    )
    L.append(
        "-- legal claim (CHG-107 tracks the sign-off question). A seeded row, so the"
    )
    ...
    L.append(
        "INSERT INTO app.threshold_policy (id,tenant_id,axle_class,retread_threshold_mm,scrap_threshold_mm,warning_threshold_mm,retreads_permitted,effective_from)"
    )
    L.append(
        f"  VALUES (md5('{tid}thrpol-steer')::uuid,'{tid}','STEER',{RETREAD_THRESHOLD_MM},{SCRAP_THRESHOLD_MM},{WARNING_THRESHOLD_MM},false,'-infinity');"
    )
    L.append("")
    L.append(
        "-- CHG-112/CHG-034: target_pressure is the one pressure-target source; no"
    )
    ...
    L.append(
        "-- unclassifiable, never silently compliant. TAG does carry a row: unlike"
    )
    L.append(
        "-- a spare it is a running road-contact position, and without a target its"
    )
    L.append(
        "-- readings would be unclassifiable and FR-INS-031a's capture-time warning"
    )
    L.append(
        "-- silently disabled, latent until the first tag-axle trailer, then live."
    )
    for cls, kpa in TARGET_KPA.items():
        L.append(
            "INSERT INTO app.target_pressure (id,tenant_id,axle_class,target_kpa,warn_under_pct,critical_under_pct,warn_over_pct,critical_over_pct,effective_from)"
        )
        L.append(
            f"  VALUES (md5('{tid}tgtp-{cls}')::uuid,'{tid}','{cls}',{kpa},10.0,20.0,10.0,20.0,'2024-01-01T00:00:00Z');"
        )
    L.append("")
    ...
    for code, name, sev in [
        ("FR-EXC-020", "Tread below removal threshold", "CRITICAL"),
        ...
        L.append(
            f"INSERT INTO app.exception_rule (id,tenant_id,code,name,enabled,severity) VALUES (md5('{tid}excr-{code}')::uuid,'{tid}','{code}',$${name}$$,true,'{sev}');"
        )
    L.append("")
    for code, name, kind, status, axles in UNITS:
        rows = build(axles)
        L.append(
            "INSERT INTO app.axle_configuration (id,tenant_id,code,name,axle_count,evidential_status,default_spare_count)"
        )
        L.append(
            f"  VALUES (md5('{tid}{code}')::uuid,'{tid}','{code}',$${name}$$,{len(axles)},'{status}',1);"
        )
        vals = []
        for r in rows:
            vals.append(
                f"('{tid}',md5('{tid}{code}')::uuid,'{r['own']}',{r['own']},{r['axle']},'{r['cls']}','{r['side']}','{r['slot']}',false,'FIXED',NULL,$${name}$$)"
            )
        ...
        vals.append(
            f"('{tid}',md5('{tid}{code}')::uuid,'S',{len(rows) + 1},NULL,'SPARE',NULL,NULL,true,'FIXED',1,$${name}$$)"
        )
        L.append(
            "INSERT INTO app.position (tenant_id,configuration_id,code,sequence,axle_number,axle_class,side,slot,is_spare,axle_type,spare_ordinal,unit_label) VALUES"
        )
