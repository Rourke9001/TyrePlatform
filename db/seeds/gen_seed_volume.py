...
HORSE = [(str(i), "STEER" if i <= 2 else "DRIVE") for i in range(1, 11)] + [
    ("S", "SPARE")
]
...
for ukey, r, seq, fleet, reg, cfg, kind, codes in units:
    for code, cls in codes:
        ...
        state[(ukey, code)] = dict(
            cls=cls,
            mm=start,
            rate=rng.uniform(lo, hi),
            offs=rng.choices(OFFSETS, weights=OFFSET_WEIGHTS)[0],
            tyre=key,
            fit=f,
        )
        ...
        for ukey, rr, seq, fleet, reg, cfg, kind, codes in units:
            ...
            for code, cls in codes:
                ...
                if cls != "SPARE" and gov <= RETREAD_THRESHOLD_MM:
                    ...
                    s["fit"].update(
                        removed=d, removed_odo=this_odo, removed_mm=float(gov)
                    )
...
L = [
    "-- Seed: volume tenant for the dashboard read-path measurement (TYRE-247).",
...
L.append(
    f"INSERT INTO app.tyre_size (id,tenant_id,name,construction) VALUES (md5('sbvol-sz1')::uuid,'{T3}','315/80R22.5','RADIAL');"
)
L.append(
    f"INSERT INTO app.tyre_brand (id,tenant_id,name) VALUES (md5('sbvol-br1')::uuid,'{T3}','Sandbox Brand');"
)
L.append(
    f"INSERT INTO app.tyre_pattern (id,tenant_id,name,brand_id) VALUES (md5('sbvol-pt1')::uuid,'{T3}','SBX1',md5('sbvol-br1')::uuid);"
)
L.append("")
...
for ukey, r, seq, fleet, reg, cfg, kind, codes in units:
    ...
    L.append(
        f"  VALUES (md5('{ukey}')::uuid,'{T3}','{fleet}','{reg}',md5('{T3}{cfg}')::uuid,'{kind}','Flat deck',{DEPOT},{sqlnull(cur)},'ACTIVE');"
    )
L.append("")
for r in range(1, RIGS + 1):
    L.append(
        f"INSERT INTO app.combination (id,tenant_id,motive_vehicle_id,effective_from) VALUES (md5('sbvol-comb{r}')::uuid,'{T3}',md5('sbvol-h{r}')::uuid,'{ts(FIRST_FIT, 6, 0)}');"
    )
    for ukey, rr, seq, fleet, reg, cfg, kind, codes in units:
        if rr == r:
            label = "Horse" if seq == 1 else ("Link A" if seq == 2 else "Link B")
            L.append(
                f"INSERT INTO app.combination_member (tenant_id,combination_id,vehicle_id,sequence,descriptor) VALUES ('{T3}',md5('sbvol-comb{r}')::uuid,md5('{ukey}')::uuid,{seq},'{label}');"
            )
L.append("")
...
for t in tyres:
    ...
    L.append(
        f"  VALUES (md5('{t['key']}')::uuid,'{T3}','{t['key'].upper()}',md5('sbvol-sz1')::uuid,md5('sbvol-br1')::uuid,md5('sbvol-pt1')::uuid,'NEW','{t['purchased']}','{t['purchased']}',{t['price']},'INVOICE',{n1(NEW_TREAD_MM)},app.rand_per_mm({t['price']},{n1(NEW_TREAD_MM)},{n1(RETREAD_THRESHOLD_MM)}),{t['casing']},'{state_}');"
    )
L.append("")
...
for f in fitments:
    ...
    if f["removed"] is None:
        ...
        L.append(
            f"  SELECT '{T3}',md5('{tkey}')::uuid,md5('{f['unit']}')::uuid,p.position_id,'{fitted_at}',{sqlnull(f['fitted_odo'])},{n1(f['fitted_mm'])}"
        )
    else:
        removed_at = ts(f["removed"], 6, 0)
        L.append(
            "INSERT INTO app.fitment (tenant_id,tyre_id,vehicle_id,position_id,fitted_at,fitted_odometer,fitted_tread_mm,removed_at,removed_odometer,removed_tread_mm,removal_reason)"
        )
        L.append(
            f"  SELECT '{T3}',md5('{tkey}')::uuid,md5('{f['unit']}')::uuid,p.position_id,'{fitted_at}',{sqlnull(f['fitted_odo'])},{n1(f['fitted_mm'])},'{removed_at}',{sqlnull(f['removed_odo'])},{n1(f['removed_mm'])},'worn_to_threshold'"
        )
    L.append(
        f"    FROM sbvol_pos p WHERE p.vehicle_id = md5('{f['unit']}')::uuid AND p.code = '{f['code']}';"
    )
L.append("")
L.append(
    "-- Each reading's lowest measurement is emitted first: reading_measurement_governs"
)
L.append(
    "-- (000001) recomputes MIN per row and reading_snapshots_governing_change (000006)"
)
L.append(
    "-- reconciles a snapshot per change, so the load order, not the data, keeps that"
)
L.append(
    "-- to one reconcile per reading. The ordinal check (000046) runs once per statement."
)
for r, k, d, horse_odo, rows in inspections:
    ...
    L.append(
        f"  VALUES (md5('{ikey}')::uuid,'{T3}',md5('sbvol-h{r}')::uuid,md5('sbvol-comb{r}')::uuid,{DRIVER},md5('sbvol-cli{r}-{k}')::uuid,'{ts(d, 5, 38)}','{ts(d, 5, 45)}',{horse_odo},'seed-volume','0.0.0-volume',420);"
    )
    vals = []
    for seq_i, (ukey, code, tyre, kpa, mm) in enumerate(rows):
        vals.append(
            f"(md5('sbvol-rd{r}-{k}-{ukey}-{code}')::uuid,md5('{ukey}')::uuid,'{code}',md5('{tyre}')::uuid,{kpa})"
        )
    L.append(
        "INSERT INTO app.reading (id,tenant_id,inspection_id,vehicle_id,position_id,tyre_id,pressure_kpa)"
    )
    L.append(
        f"  SELECT x.id,'{T3}',md5('{ikey}')::uuid,x.vehicle_id,p.position_id,x.tyre_id,x.kpa"
    )
    L.append(
        "    FROM (VALUES " + ",".join(vals) + ") AS x(id,vehicle_id,code,tyre_id,kpa)"
    )
    ...
    for ukey, code, tyre, kpa, mm in rows:
        ...
        for i in sorted(range(3), key=lambda i: (mm[i], i)):
            mvals.append(
                f"('{T3}',{rd},{i + 1},'{POSITIONS[i]}',{n1(mm[i])},{known},1.0)"
            )
    L.append(
        "INSERT INTO app.reading_measurement (tenant_id,reading_id,ordinal,position,tread_mm,orientation_known,granularity_mm) VALUES"
    )
    L.append("  " + ",\n  ".join(mvals) + ";")
L.append("")
L.append("COMMIT;")
...
print(
    f"volume written: {len(units)} units, {len(tyres)} tyres, {len(fitments)} fitments, {len(inspections)} inspections"
)
