...
-- user_depot is tenant data, recording who may see which depot, so DR-001
-- requires a non-nullable tenant key, and the standard policy shape needs the
-- column to bind RLS at all. The backfill resolves through depot, whose
-- tenant_id is NOT NULL; app_user's cannot anchor it because PLATFORM_ADMIN
-- rows carry NULL there (FR-AUT-003).
...
-- connection. INSERT and UPDATE stay, because ORG_ADMIN invites, suspends and
