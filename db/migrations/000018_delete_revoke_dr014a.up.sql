...
--  That is the CR-004/DR-011 principle 000001 already enforces with
--  UPDATE/DELETE revokes on reading, reading_measurement, tyre_event and
--  audit_log, and 000012 extends to casing_valuation, tenant_consent and
...
--
--  DR-014b is its own ticket, TYRE-60: soft-delete (an `active`-style flag)
--  for the handful of tables where a manager needs to retract a mistaken row
--  without an event log (vehicle_tag, operating_group membership, and
--  similar). Nothing here does that; it only removes the capability
--  to hard-delete.
--
