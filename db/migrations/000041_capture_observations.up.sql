-- 000041: what a capture observes beyond its readings. A spare position the
-- unit does not carry is recorded as an observation on the inspection
...
-- ---------------------------------------------------------------------------
-- TYRE-155. One row per (inspection, unit, spare position) the driver said
-- was empty. The configuration keeps seeding one spare cell per unit so a
-- real but unregistered spare can still be captured; this row is what lets
-- completeness reach 100% on a unit that carries none, and what the
-- controller reconciles against the register (B6.4, TYRE-75).
CREATE TABLE app.inspection_absent_spare (
  ...
  FOREIGN KEY (tenant_id, vehicle_id)    REFERENCES app.vehicle (tenant_id, id),
  FOREIGN KEY (tenant_id, position_id)   REFERENCES app.position (tenant_id, id),
  FOREIGN KEY (tenant_id, created_by)    REFERENCES app.app_user (tenant_id, id),
  UNIQUE (tenant_id, inspection_id, vehicle_id, position_id)
);
CREATE INDEX absent_spare_by_inspection ON app.inspection_absent_spare (tenant_id, inspection_id);

CALL app.enable_tenant_rls('app.inspection_absent_spare'::regclass);
GRANT SELECT, INSERT ON app.inspection_absent_spare TO app_rw;
-- Rule 3: a record of fact is enforced by grant, not convention.
REVOKE UPDATE, DELETE ON app.inspection_absent_spare FROM app_rw;

COMMENT ON TABLE app.inspection_absent_spare IS
  'A spare position the driver reported empty at this inspection (FR-INS-066 v1.4). An observation, not a fitment event: the controller reconciles it against the register.';
...
BEGIN
  ...
  FOR a IN SELECT * FROM jsonb_array_elements(COALESCE(p_payload -> 'absent_spares', '[]'::jsonb)) LOOP
    SELECT p.is_spare INTO v_spare
      FROM app.position p
      JOIN app.axle_configuration ac ON ac.id = p.configuration_id
     WHERE p.id = (a ->> 'position_id')::uuid
       AND p.tenant_id = v_tenant
       AND (ac.tenant_id, ac.code) = (
             SELECT a2.tenant_id, a2.code
               FROM app.vehicle v JOIN app.axle_configuration a2 ON a2.id = v.configuration_id
              WHERE v.id = (a ->> 'vehicle_id')::uuid AND v.tenant_id = v_tenant);
    IF NOT FOUND THEN
      RAISE EXCEPTION 'position % is not on any version of vehicle %''s configuration',
        a ->> 'position_id', a ->> 'vehicle_id' USING ERRCODE = 'TY004';
    END IF;
    IF NOT v_spare THEN
      RAISE EXCEPTION 'position % is not a spare; only a spare can be reported absent',
        a ->> 'position_id' USING ERRCODE = 'TY005';
    END IF;
    IF EXISTS (SELECT 1 FROM app.reading r
                WHERE r.inspection_id = v_insp
                  AND r.vehicle_id = (a ->> 'vehicle_id')::uuid
                  AND r.position_id = (a ->> 'position_id')::uuid) THEN
      RAISE EXCEPTION 'position % carries a reading and cannot also be absent',
        a ->> 'position_id' USING ERRCODE = 'TY005';
    END IF;
    INSERT INTO app.inspection_absent_spare (tenant_id, inspection_id, vehicle_id, position_id)
    VALUES (v_tenant, v_insp, (a ->> 'vehicle_id')::uuid, (a ->> 'position_id')::uuid);
  END LOOP;
