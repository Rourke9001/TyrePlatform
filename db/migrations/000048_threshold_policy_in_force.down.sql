-- 000048 down. Restores the state the up migration found: v_removal_forecast
-- as 000013 left it, current_removal_threshold_mm as 000006 left it, and the
-- three writers as 000039 left them. No later ALTER, comment or grant names
-- any of them, so each CREATE OR REPLACE below is that object's whole state
-- (docs/lessons.md, 2026-09-15), and replacing rather than dropping keeps
-- their oids and grants. The view goes first because it is the one object
-- pg_depend ties to app.threshold_policy_in_force, which goes last.
...
DROP FUNCTION app.threshold_policy_in_force(uuid, uuid, app.axle_class);
